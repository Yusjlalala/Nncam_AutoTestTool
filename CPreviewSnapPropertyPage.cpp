...
void CPreviewSnapPropertyPage::UpdateSnapRes()
{
	if (g_hCam)
	{
		int resCnt = Nncam_get_StillResolutionNumber(g_hCam);
		...
		else
		{
			...
			{
				Nncam_get_StillResolution(g_hCam, i, &width, &height);
				CString str;
				str.Format(_T("%d x %d"), width, height);
				resList->AddString(str);
			}
...
BOOL CPreviewSnapPropertyPage::OnInitDialog()
{
	...
	if (g_hCam)
	{
		int resCnt = Nncam_get_ResolutionNumber(g_hCam);
		CComboBox* previewResList = (CComboBox*)GetDlgItem(IDC_COMBO_PREVIEW);
		int width = 0, height = 0;
		for (int i = 0; i < resCnt; ++i)
		{
			Nncam_get_Resolution(g_hCam, i, &width, &height);
			CString str;
			str.Format(_T("%d x %d"), width, height);
			previewResList->AddString(str);
		}
		unsigned resIndex = 0;
		Nncam_get_eSize(g_hCam, &resIndex);
...
void CPreviewSnapPropertyPage::OnBnClickedButtonSnap()
{
	...
	{
		unsigned res = 0;
		Nncam_get_eSize(g_hCam, &res);
		Nncam_Snap(g_hCam, res);
	}
	else
	{
		CComboBox* resList = (CComboBox*)GetDlgItem(IDC_COMBO_SNAP);
		Nncam_Snap(g_hCam, resList->GetCurSel());
	}
}
