...
LRESULT CAutoTestToolDlg::OnMsgCamevent(WPARAM wp, LPARAM lp)
{
	...
	case NNCAM_EVENT_NOFRAMETIMEOUT:
...
LRESULT CAutoTestToolDlg::OnPreviewResChanged(WPARAM wp, LPARAM lp)
{
	...
	HRESULT hr = Nncam_get_eSize(g_hCam, &nResolutionIndex);
	if (FAILED(hr))
		return -1;

	if (nResolutionIndex != nSel)
	{
		hr = Nncam_Stop(g_hCam);
		if (FAILED(hr))
			return -1;
		Nncam_put_eSize(g_hCam, nSel);
...
{
	g_cameraCnt = Nncam_EnumV2(g_cameras);
...
void CAutoTestToolDlg::StartCamera()
{
	...
	Nncam_get_Size(g_hCam, &width, &height);
...
void CAutoTestToolDlg::OnEventImage()
{
	...
	HRESULT hr = Nncam_PullImageV2(g_hCam, m_pImageData, 24, &info);
	if (SUCCEEDED(hr))
	{
		if (g_bROITesting && g_bROITest_StartSnap)
		{
			unsigned offsetX = 0, offsetY = 0,  width = 0, height = 0;
			Nncam_get_Roi(g_hCam, &offsetX, &offsetY, &width, &height);
		...
		else if (g_bImageShoot)
		{
			...
			Nncam_get_Size(g_hCam, &resWidth, &resHeight);
			CString str;
			SYSTEMTIME tm;
			GetLocalTime(&tm);
...
{
	NncamFrameInfoV2 info = { 0 };
	HRESULT hr = Nncam_PullStillImageV2(g_hCam, nullptr, 24, &info);
	if (SUCCEEDED(hr))
	{
		void* pData = malloc(TDIBWIDTHBYTES(info.width * 24) * info.height);
		hr = Nncam_PullStillImage(g_hCam, pData, 24, nullptr, nullptr);
		if (SUCCEEDED(hr))
		{
			BITMAPINFOHEADER header = { 0 };
			header.biSize = sizeof(header);
...
void CAutoTestToolDlg::OnBnClickedButton1()
{
	if (g_cameraCnt <= 0)
		return;
	Nncam_Replug(g_cameras[m_cameraList.GetCurSel()].id);
}
