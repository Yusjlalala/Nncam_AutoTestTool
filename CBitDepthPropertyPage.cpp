...
BOOL CBitDepthPropertyPage::OnInitDialog()
{
	...
	if (g_hCam)
	{
		int maxBit = Nncam_get_MaxBitDepth(g_hCam);
		if (maxBit <= 8)
		...
		Nncam_get_Option(g_hCam, NNCAM_OPTION_BITDEPTH, &m_bitDepth);
		UpdateData(FALSE);
	}
...
{
	UpdateData(TRUE);
	Nncam_put_Option(g_hCam, NNCAM_OPTION_BITDEPTH, m_bitDepth);
}
